...
plt.plot(pclass_survived_mean['Pclass'], pclass_survived_mean['Survived'],
         marker ='o', linestsyle ='-', color = 'violet')
...
plt.title('Correlation Heatmp of Titanic')
...
#결측치 처리
titanic = titanic.drpona(subset = ['Age'])
print(titanic.info())
...
#showmeans=False 는 평균값을 표시하지 않도록 하고, showmedians=True는 중앙값을 표시하도록 함
...
# 범례 설정
plt.legned(violin_plot['bodies'], ['1st Class', '2nd Class', '3rd Class'],
...
#Survived가 - 이면 사망자를 ,1이면 생존자를 나타냄
titanic = pd.read_csv('3.1.1.titanic.csv')
...
#제목 설정
fig.suptitle('Survival Analysis by Number of Parents/Childer (Parch) on the Titanic')
fig.tight_layout()
...
#.twinx() 함수를 사용하여 두 번째 그래프와 축을 공유하느 새로운 Axes객체 생성
ax2 = ax1.twinx()

# 두 번째 그래프 설정(막대 그래프)
ax2.bar(x,y2,color='deeppink', alpha = 0.7, width = 0.7, label = 'Survived')
ax2.set_ylabel('Survived Coubt', color = 'deeppink') #서브플롯 y축에 레이블 추가
ax2.tick_params(axis='y', labelcolor='deeppink')
ax2.legend(loc = 'upper right', bbox_to_anchor = (1,0.9))

plt.suptitle('Surviva Analysis by Number of Parents/Children (parch) on the Titanic')
plt.tight_layout()
...
plt.xlabel('Age')
plt.ylabel('Count')
plt.title('Distrivution of Ages on the Titanic')
plt.grid(axis = 'y',linestyle = '--', alpha = 0.7)
