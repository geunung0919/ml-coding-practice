...
netflix['director'] = netflix['director'].reaplce(np.nan, 'No data')
...
# insa() == isnull() : 결측 값은 True 반환하고, 그 외에는 False 반환
netflix.insa().sum()
...
plt.title('We see more movies than TV Show on Netflix.', fontfamliy='serif', fontsize=12)
plt.show()
